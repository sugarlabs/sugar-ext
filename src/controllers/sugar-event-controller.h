...
#if !defined (__SUGAR_CONTROLLERS_H_INSIDE__) && !defined (SUGAR_TOOLKIT_COMPILATION) && 0
...
typedef enum {
  ...
  SUGAR_EVENT_CONTROLLER_FLAG_EMULATE_TOUCH = 1 << 0,
  SUGAR_EVENT_CONTROLLER_FLAG_EXCLUSIVE = 1 << 1
...
gboolean  sugar_event_controller_detach       (SugarEventController      *controller,
                                               GtkWidget                 *widget);
GtkWidget *sugar_event_controller_get_widget (SugarEventController *controller);
SugarEventControllerFlags sugar_event_controller_get_flags (SugarEventController *controller);
...
G_DEFINE_AUTOPTR_CLEANUP_FUNC(SugarEventController, g_object_unref)
